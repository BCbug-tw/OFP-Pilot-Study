...
class Config:
    # Model
    IMAGE_SIZE = 224
    NUM_CLASSES = 1  # Binary Infiltration
    ...
    # Training - ViT usually requires a smaller learning rate when fine-tuning
    ...
    USE_LLRD = False  # Set to True to enable Layer-Wise Learning Rate Decay (LLRD) for ViT
    USE_LORA = True # Set to True to enable LoRA for ViT
    ...
    PRETRAIN_CHECKPOINT = os.path.join(DAPT_DIR, "mae_vit_dapt_best.pth") if USE_DAPT else None
    ...
    CHECKPOINT_PATH = os.path.join(OUTPUT_DIR, f"vit_finetune_best_{exp_name}.pth")
