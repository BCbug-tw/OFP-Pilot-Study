...
def plot_roc_curve(y_true, y_score, output_dir, class_names, display_name, save_name):
    ...
    plt.title(f'ROC Curve for ViT with {display_name} on ChestMNIST')
    plt.legend(loc="lower right", fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"ViT_roc_curve_{save_name}.png"), dpi=300)
    plt.close()
...
def plot_pr_curve(y_true, y_score, output_dir, class_names, display_name, save_name):
    ...
    plt.title(f'PR Curve for ViT with {display_name} on ChestMNIST')
    plt.legend(loc="lower left", fontsize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f"ViT_pr_curve_{save_name}.png"), dpi=300)
    plt.close()

def reshape_transform(tensor, height=14, width=14):
    # ViT B/16 extracts 14x14 patches (from 224x224 imgs) and a CLS token.
    # Output of target layer normally has shape (batch_size, 197, 768).
    # We strip out the CLS token (first token) and reshape the rest into 14x14 grid.
...
def generate_gradcam(model, dataset, device, output_dir, class_names, samples_per_class=2):
    # For ViT, we typically target the final LayerNorm block in the encoder
    target_layers = [model.mae.vit.encoder.layers[-1].ln_1]
...
def evaluate():
    ...
    print(f"Initializing {display_model_type} ViT model...")
    ...
    print(f"ROC curve saved to {os.path.join(output_dir, f'ViT_roc_curve_{exp_name}.png')}")
    
    print("Plotting PR curves...")
    plot_pr_curve(y_true, y_score, output_dir, class_names_list, display_model_type, exp_name)
    print(f"PR curve saved to {os.path.join(output_dir, f'ViT_pr_curve_{exp_name}.png')}")
    ...
    # NOTE: pytorch-grad-cam requires layers requiring grad, let's explicitly enable them during eval for CAM computation
