...
def train():
    ...
    train_dataset = ChestMNISTDataset(split='train', stage='train', img_size=Config.IMAGE_SIZE)
    val_dataset = ChestMNISTDataset(split='val', stage='val', img_size=Config.IMAGE_SIZE)
    ...
    logger.info(f"Initializing Fine-Tuning {model_type} Swin-B Classifier...")
    ...
    from utils import get_llrd_param_groups, get_param_groups_with_head_lr, get_cosine_schedule_with_warmup
    if hasattr(Config, 'USE_LORA') and Config.USE_LORA:
        ...
        logger.info(f"Using standard learning rate ({Config.LR}) with Head LR Multiplier ({Config.HEAD_LR_MULTIPLIER}).")
        optimizer_groups = get_param_groups_with_head_lr(model, Config.LR, Config.HEAD_LR_MULTIPLIER, Config.WEIGHT_DECAY)
        optimizer = torch.optim.Adam(optimizer_groups)
    
    accum_steps_total = getattr(Config, 'GRAD_ACCUM_STEPS', 1)
    num_training_steps = (Config.EPOCHS * len(train_loader)) // accum_steps_total
    num_warmup_steps = (Config.WARMUP_EPOCHS * len(train_loader)) // accum_steps_total
