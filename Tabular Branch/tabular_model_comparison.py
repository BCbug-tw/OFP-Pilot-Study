...
# ---------------------------------------------------------
# Configuration: Define the tabular models to compare here
# ---------------------------------------------------------
MODELS_TO_COMPARE = [
    {
        "name": "XGBoost",
        "prediction_file": "XGBoost/test_predictions.csv",
    },
    {
        "name": "LightGBM",
        "prediction_file": "LightGBM/test_predictions.csv",
    },
    {
        "name": "CatBoost",
        "prediction_file": "CatBoost/test_predictions.csv",
    },
    {
        "name": "FT-Transformer",
        "prediction_file": "FT-Transformer/test_predictions.csv",
    }
]

TARGET_CLASS = "Diabetes_binary" 
OUTPUT_DIR = "tabular_comparison_results"
...
def main():
    ...
    for idx, model_config in enumerate(MODELS_TO_COMPARE):
        ...
        if not os.path.exists(file_path):
            print(f"Warning: File not found for {name} ({file_path}). Skipping this model.")
            continue
            
        print(f"Processing {name}...")
        df = pd.read_csv(file_path)
        
        # Determine column names based on model output format
        # Baseline scripts output GroundTruth_Diabetes_binary, Prob_Diabetes_binary, Pred_Diabetes_binary
        # FT-Transformer outputs True_Label, Predicted_Probability, Predicted_Label
        if 'GroundTruth_' + TARGET_CLASS in df.columns:
            gt_col = f"GroundTruth_{TARGET_CLASS}"
            prob_col = f"Prob_{TARGET_CLASS}"
            pred_col = f"Pred_{TARGET_CLASS}"
        else:
            # Fallback for FT-Transformer format
            gt_col = "True_Label"
    ...
    roc_ax.set_title(f'Integrated ROC Curve - {TARGET_CLASS}', fontsize=14)
    roc_ax.legend(loc="lower right", fontsize=10)
    roc_ax.grid(alpha=0.3)
    roc_figure_path = os.path.join(OUTPUT_DIR, "Tabular_Integrated_ROC_Curve.png")
    ...
    pr_ax.set_title(f'Integrated PR Curve - {TARGET_CLASS}', fontsize=14)
    pr_ax.legend(loc="lower right", fontsize=10)
    pr_ax.grid(alpha=0.3)
    pr_figure_path = os.path.join(OUTPUT_DIR, "Tabular_Integrated_PR_Curve.png")
    ...
    print("\n============== Tabular Model Comparison Metrics ==============")
    print(results_df.to_string(index=False))
    print("================================================================\n")
    
    # CSV Table Output
    csv_path = os.path.join(OUTPUT_DIR, "Tabular_Model_Comparison_Metrics.csv")
    ...
    plt.title(f'Tabular Model Comparison Metrics - {TARGET_CLASS}', fontsize=14, pad=20)
    plt.tight_layout()
    table_img_path = os.path.join(OUTPUT_DIR, "Tabular_Model_Comparison_Metrics.png")
