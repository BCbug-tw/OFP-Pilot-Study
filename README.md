...
*   **詳細成果**：完整的超參數設定、訓練集分割策略以及各種模型彼此間的效能長短處比較（如 AUC、Accuracy 等），請參閱 [`結構化資料 (Tabular) 模型實驗成果與比較分析報告`](./Tabular%20Branch/README.md)。
...
*   **詳細成果**：實驗中所使用的影像處理手法、各階段微調機制的差異、以及最終影像分類指標的比較，請參閱 [`影像模型實驗成果與比較分析報告`](./Image%20Branch/README.md)。
